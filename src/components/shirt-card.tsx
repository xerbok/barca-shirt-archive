...
export function ShirtCard({ shirt }: ShirtCardProps) {
  const imageLabel = `Imatge principal de la samarreta ${shirt.season} ${shirt.shirt_type}`;
      ...
      <article>
        {shirt.main_image ? (
          <div
            ...
            className="relative aspect-[4/3] overflow-hidden bg-slate-200 bg-cover bg-center"
            role="img"
            style={{
        ...
        <div className="space-y-5 p-5">
          ...
          <dl className="grid grid-cols-2 gap-3 text-sm">
            <InfoItem
              label="Jugador"
              value={shirt.player_name ?? "Sense nom"}
            />
            <InfoItem
              label="Dorsal"
              value={
                shirt.number === null ? "Sense dorsal" : String(shirt.number)
              }
            />
